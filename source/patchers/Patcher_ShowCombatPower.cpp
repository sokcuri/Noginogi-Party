...
LPBYTE CPatcher_ShowCombatPower::addrIsGoodNPC = NULL;
LPBYTE CPatcher_ShowCombatPower::addrGetLife = NULL;
BYTE   CPatcher_ShowCombatPower::callBackup[4]    = { 0x00, 0x00, 0x00, 0x00 };

bool CPatcher_ShowCombatPower::ShowCombatNPCGrade = false;
bool CPatcher_ShowCombatPower::ShowCombatGoodNPCGrade = false;
bool CPatcher_ShowCombatPower::ShowCombatPlayerGrade = false;
...
CPatcher_ShowCombatPower::CPatcher_ShowCombatPower( void )
{
	...
	if (!mp1.Search( L"Pleione.dll" )) {
		...
		0xFF, 0x90, -1, -1, -1, -1,			// 4C: CALL DWORD PTR DS:[EAX+1F0] (자기자신인지 확인)
	...
	MemoryPatch mp( NULL, backup2, backup2 );
	if (!mp.Search( L"Pleione.dll" )) {
		WriteLog("패치 초기화 실패: %s.\n", patchname.c_str());
		return;
	}
	MemoryPatch::ReadMem( mp.GetAddr() + 0x30, callBackup, 4 );
...
NAKED void CPatcher_ShowCombatPower::patchShowCombatPower(void)
{
	...
	//		CALL	addrGetLife
			...
			PUSH EBX
			MOV EBX, EAX
			ADD EBX, DWORD PTR DS:[callBackup]
	...
	{
		__asm JMP		addrGetTargetCombatPower
	}
}
...
NAKED void CPatcher_ShowCombatPower::patchIsCurrentPlayer(void)
{
	...
	// 스카우터 Off
		...
		__asm {
			POP		EAX
			ADD		EAX, DWORD PTR DS:[callBackup]
...
void __stdcall CPatcher_ShowCombatPower::patchConnectFormat2(wchar_t * buff,wchar_t * str1,wchar_t * str2)
{
	swprintf_s(buff,255,L"%s ",str1);
}
//-----------------------------------------------------------------------------
// INI Functions
...
bool CPatcher_ShowCombatPower::ReadINI( void )
{
		...
		ShowCombatNPCGrade = ReadINI_Bool( L"ShowCombatNPCGrade");
		ShowCombatGoodNPCGrade = ReadINI_Bool ( L"SHowCombatGoodNPCGrade" );
		ShowCombatPlayerGrade = ReadINI_Bool( L"ShowCombatPlayerGrade");

		WriteLog("ShowCombatNPCGrade: %d\n", ShowCombatNPCGrade);
		WriteLog("ShowCombatPlayerGrade: %d\n", ShowCombatPlayerGrade);
